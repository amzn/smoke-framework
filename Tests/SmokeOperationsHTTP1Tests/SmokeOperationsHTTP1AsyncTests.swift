...
private let testSleepNs: UInt64 = 1000

private func handleExampleOperationVoid(input: ExampleInput, context: ExampleContext) async throws {
    try await Task.sleep(nanoseconds: testSleepNs)
    
    // This function intentionally left blank.
}

private func handleExampleHTTP1OperationVoid(input: ExampleHTTP1Input, context: ExampleContext) async throws {
    try await Task.sleep(nanoseconds: testSleepNs)
    
    input.validateForTest()
}

private func handleBadOperationVoid(input: ExampleInput, context: ExampleContext) async throws {
    try await Task.sleep(nanoseconds: testSleepNs)
    
    throw MyError.theError(reason: "Is bad!")
}

private func handleBadHTTP1OperationVoid(input: ExampleHTTP1Input, context: ExampleContext) async throws {
    try await Task.sleep(nanoseconds: testSleepNs)
...
private func handleExampleOperation(input: ExampleInput, context: ExampleContext) async throws -> OutputAttributes {
    try await Task.sleep(nanoseconds: testSleepNs)
...
private func handleExampleHTTP1Operation(input: ExampleHTTP1Input, context: ExampleContext) async throws -> OutputHTTP1Attributes {
    try await Task.sleep(nanoseconds: testSleepNs)
...
private func handleBadOperation(input: ExampleInput, context: ExampleContext) async throws -> OutputAttributes {
    try await Task.sleep(nanoseconds: testSleepNs)
    
    throw MyError.theError(reason: "Is bad!")
}

private func handleBadHTTP1Operation(input: ExampleHTTP1Input, context: ExampleContext) async throws -> OutputHTTP1Attributes {
    try await Task.sleep(nanoseconds: testSleepNs)
