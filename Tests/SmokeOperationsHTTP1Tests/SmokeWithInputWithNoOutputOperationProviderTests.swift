...
//
// SmokeWithInputNoOutputOperationProviderTests.swift
...
private extension ExampleContext {
    @Sendable func successOperation(_ input: ExampleHTTP1Input) {
    ...
    @Sendable func allowedFailureOperation(_ input: ExampleHTTP1Input) throws {
        throw TestErrors.allowedError
    }
    
    @Sendable func notAllowedFailureOperation(_ input: ExampleHTTP1Input) throws {
        throw TestErrors.notAllowedError
    }
}

class SmokeWithInputNoOutputOperationProviderTests: XCTestCase {
    ...
    func testSuccessNoInnerMiddlewareNoOuterMiddleware() async throws {
        ...
        // the writer should be completed, with the success status and with no response body as expected
