...
//
// SmokeWithNoInputTypedTransformWithOutputOperationProviderTests.swift
...
private extension ExampleContext{
    @Sendable func successOperation(_ input: HTTPServerRequest) async throws -> OutputHTTP1Attributes {
    ...
    @Sendable func successOperation2<Input: BodyProvider>(_ input: Input) async throws -> OutputHTTP1Attributes {
        let bodyByteBuffer = try await input.body.collect(upTo: TestValues.maxBodySize)
        let bodyAsString = String(buffer: bodyByteBuffer)
    ...
    @Sendable func allowedFailureOperation(_ input: HTTPServerRequest) throws -> OutputHTTP1Attributes {
        throw TestErrors.allowedError
    }
    
    @Sendable func notAllowedFailureOperation(_ input: HTTPServerRequest) throws -> OutputHTTP1Attributes {
        throw TestErrors.notAllowedError
    }
}

class SmokeWithNoInputTypedTransformWithOutputOperationProviderTests: XCTestCase {
