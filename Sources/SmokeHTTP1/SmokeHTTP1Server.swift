...
public class SmokeHTTP1Server {
    ...
    private let startHandler: () throws -> Void
    private let shutdownHandler: () throws -> Void
    private let waitUntilShutdownHandler: () throws -> Void
    private let waitUntilShutdownAndThenHandler: (_ onShutdown: @escaping () -> Void) throws -> Void
    private let onShutdownHandler: (_ onShutdown: @escaping () -> Void) throws -> Void

    public init<HTTP1RequestHandlerType: HTTP1RequestHandler>(wrappedServer: StandardSmokeHTTP1Server<HTTP1RequestHandlerType>) {
        self.startHandler = {
