...
//
//  NoRequestTransformMiddleware.swift
...
public struct NoRequestTransformMiddleware<IncomingOutputWriter: HTTPServerResponseWriterProtocol,
    ...
    public func handle(_ input: HTTPServerRequest,
                       ...
                       context: Context,
                       next: (HTTPServerRequest, OutgoingOutputWriter, Context) async throws -> Void) async throws {
        let outgoingOutputWriter = self.outputWriterTransformer(outputWriter)
        
        try await next(input, outgoingOutputWriter, context)
    }
}
